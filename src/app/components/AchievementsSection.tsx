...
const ResponsiveCarousel = () => {
  ...
  return (
    <div className="relative w-full max-w-[750px] h-[200px] sm:h-[240px] lg:h-[280px] px-4 sm:px-0">
      ...
      {currentSlide > 0 && (
        <motion.button
          className="absolute left-6 sm:left-8 top-1/2 -translate-y-1/2 w-8 h-8 sm:w-10 sm:h-10 bg-black/20 hover:bg-black/30 rounded-full flex items-center justify-center z-10 border-none cursor-pointer overflow-hidden backdrop-blur-sm"
        ...
        >
          <svg width="16" height="16" className="sm:w-6 sm:h-6" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      {currentSlide < carouselImages.length - 1 && (
        <motion.button
          className="absolute right-6 sm:right-8 top-1/2 -translate-y-1/2 w-8 h-8 sm:w-10 sm:h-10 bg-black/20 hover:bg-black/30 rounded-full flex items-center justify-center z-10 border-none cursor-pointer overflow-hidden backdrop-blur-sm"
          onClick={goToNext}
          disabled={isTransitioning}
          whileHover={{ 
            scale: 1.15, 
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col lg:flex-row justify-center items-center w-full min-h-[316px] px-2 py-12 md:py-16 relative gap-8 lg:gap-16">
        ...
        {/* Stats Grid - Below carousel on mobile, right on desktop */}
        <motion.div 
          className="flex flex-col justify-center items-center gap-6 lg:gap-8 bg-blue-prime/20 rounded-2xl p-6 border border-white/10 backdrop-blur-sm shadow-xl"
